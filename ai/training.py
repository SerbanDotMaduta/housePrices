...
# Features:
# Target : SalePrice
# LotArea - Lot size in square feet - suprafata teren 
# GrLivArea
# BedroomAbvGr - bedroom number above ground lvl
# TotalBath - bathroom numbers
# LotFrontage - front stradal
...
plotDens = sns.distplot(train.SalePrice)
plt.show()

# Log transform the target for official scoring(skew correction)
train.SalePrice = np.log1p(train.SalePrice)
y = train.SalePrice
plotDens = sns.distplot(y)
plt.show()
...
# Modify this to train model on other inputs 
train["TotalBath"] = train["BsmtFullBath"] + (0.5 * train["BsmtHalfBath"]) + train["FullBath"] + (0.5 * train["HalfBath"])
training_filter = ["SalePrice", "LotArea", "GrLivArea", "BedroomAbvGr", "TotalBath", "LotFrontage"]

train = train[training_filter]
...
convertToSqMet = lambda x: x/10.764
convertToMet = lambda x: x/3.281

train["LotArea"] = train["LotArea"].apply(convertToSqMet)
train["GrLivArea"] =  train["GrLivArea"].apply(convertToSqMet)
train["LotFrontage"] = train["LotFrontage"].apply(convertToMet)
